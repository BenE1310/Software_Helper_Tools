...
:logo
cls
title VSIL TCS_Client Version Update 
echo.  
...
IF not exist "%~dp0..\..\Zip\TCS_Client.7z" (goto NoSource)
...
"%~dp0..\..\Tools"\"PsKill.exe" -accepteula -t FBEIronDomeVSILClient.exe
...
if not exist %DestPath%  ( goto TCS_Client
...
:TCS_Client
("%~dp0..\..\Tools\7z.exe" x "%~dp0..\..\Zip\TCS_Client.7z" -o"%DestPath%" -y) 
IF exist %DestPath% ( echo VSIL Source Folder Found ) ELSE (goto NoSource)

echo Installing VSIL TCS_Client, Please Wait...
echo.
...
echo.
echo.
echo Error! Failed to update TCS_Client folder.
echo        Make sure you are not running any process
echo.
goto eof
:NoSource
echo.
echo [%~dp0..\..\Zip\TCS_Client.7z] Not Exist
