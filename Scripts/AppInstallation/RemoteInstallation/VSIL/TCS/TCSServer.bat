...
:logo
cls
title VSIL TCS_Server Version Update 
echo.  
...
IF not exist "%~dp0..\..\Zip\TCS_Server.7z" (goto NoSource)
...
if not exist %DestPath%  ( goto TCS_Server
...
:TCS_Server
("%~dp0..\..\Tools\7z.exe" x "%~dp0..\..\Zip\TCS_Server.7z" -o"%DestPath%" -y) 
IF exist %DestPath% ( echo VSIL Source Folder Found ) ELSE (goto NoSource)

echo Installing VSIL TCS_Server, Please Wait...
echo.
echo.
echo.
...
echo.
echo.
echo Error! Failed to update TCS_Server folder.
...
:NoSource
echo.
echo [%~dp0..\..\Zip\TCS_Server.7z] Not Exist
