...
goto CreateDestPath_move

:CreateDestPath_move
FOR /L %%A IN (1,1,500) DO (
if not exist %DestPath%  ( goto DB
   ) else IF EXIST %DestPath%  ( Ren %DestPath% VSIL%mydate% 
	if errorlevel 1 goto Error_CreateDestPath
   goto DB
  goto EOF  ) )
goto DB
...
:Error_CreateDestPath
echo.
echo.
echo Error! Failed to update VSIL folder.
echo        Make sure you are not running any process
echo.
goto eof
