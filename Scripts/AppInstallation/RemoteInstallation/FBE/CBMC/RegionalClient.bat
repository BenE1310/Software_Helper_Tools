...
:logo
cls
title Firebolt Regional Client Version Update 
echo.  
...
IF not exist "%~dp0..\..\Zip\RegionalClient.7z" (goto NoSource)
...
@echo Kill Processes...
sc \\10.11.%BN%8.%PN% stop "FBE Watchdog REGIONAL"
timeout /t 10
...
"%~dp0..\..\Tools"\"PsKill.exe" -accepteula -t IronDomeFBERegionalMaintenance.exe
"%~dp0..\..\Tools"\"PsKill.exe" -accepteula -t FBEIronDomeRegionalBMCOperationalClient.exe
"%~dp0..\..\Tools"\"PsKill.exe" -accepteula -t FBEIronDomeTrainerClient.exe
...
if not exist %DestPath%  ( goto CBMC_Client
...
:CBMC_Client
("%~dp0..\..\Tools\7z.exe" x "%~dp0..\..\Zip\RegionalClient.7z" -o"%DestPath%" -y) 
IF exist %DestPath% ( echo Firebolt Source Folder Found ) ELSE (goto NoSource)

echo Installing FBE Regional Client, Please Wait...
echo.
...
sc \\10.11.%BN%8.%PN% start "FBE Watchdog REGIONAL"
goto EOF
...
echo.
echo.
echo Error! Failed to update RegionalClient folder.
echo        Make sure you are not running any process
echo.
goto eof
:NoSource
echo.
echo [%~dp0..\..\Zip\RegionalClient.7z] Not Exist
echo [%~dp0..\..\Zip\RegionalClient.7z] Not Exist
